...
renderCart();
// ============================================================
// REDISEÑO — RECORRIDO MÓVIL, NAV CON SCROLL Y REVELADOS
// ============================================================

// --- Recorrido a pantalla completa (menú móvil) ---
(function () {
  const burger = document.getElementById('navBurger');
  const menu = document.getElementById('navMenu');
  if (!burger || !menu) return;

  const items = menu.querySelectorAll('.nav-menu-list a');
  ...
  function openMenu() {
    ...
    burger.setAttribute('aria-label', 'Cerrar el recorrido');
  ...
  function closeMenu({ restoreFocus = true } = {}) {
    ...
    burger.setAttribute('aria-label', 'Abrir el recorrido');
  ...
  // cualquier sala elegida cierra el recorrido (showTab ya cambia el panel)
  menu.addEventListener('click', (e) => {
    if (e.target.closest('[data-tab]')) closeMenu({ restoreFocus: false });
  });
  ...
  // al volver a escritorio el menú no debe quedar montado
  const wide = window.matchMedia('(min-width: 1025px)');
  const onWide = (ev) => { if (ev.matches) closeMenu({ restoreFocus: false }); };
  wide.addEventListener ? wide.addEventListener('change', onWide) : wide.addListener(onWide);
})();

// --- Fondo sólido de la barra al descender ---
(function () {
...
// --- Revelado escalonado de las secciones de inicio ---
(function () {
...
// --- Altura real de la barra: alimenta --nav-h (anclajes y vista fija) ---
(function () {
...
// --- Año en curso en el aviso legal ---
(function () {
